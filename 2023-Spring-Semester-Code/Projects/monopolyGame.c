...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
        ...
        //checks property player landed on
        if(prop[p[i].loc].owner_id == -1){
            //player now owns the unowned property 
            ...
            int propertyOwnerID = prop[p[i].loc].owner_id;
            if(transaction(&p[i], &p[propertyOwnerID], rent) == 0) return 0;
        }
	}
	return 1;
}
