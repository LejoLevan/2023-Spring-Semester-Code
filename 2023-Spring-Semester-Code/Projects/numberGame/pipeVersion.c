...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    close(fdp[1]); //closes write for parent 
    close(fdc[0]); //closes read for parent

    //  send max value to the parent
    int maxValue = gmn_get_max();
    if(write(fdc[1], &maxValue, sizeof(int)) == -1) die("Write (line 120)");

    //  repeat the following until guess from parent is correct 
    int guess = 0; 
    while(guess != gmn.v){
    //  wait for a guess from parent 
        if(read(fdp[0], &guess, sizeof(int)) == -1) die("Read (line 126)");

    //  call gmn_check() 
        int result = gmn_check(&gmn, guess);
    //  send the result to parent
        if(write(fdc[1], &result, sizeof(int)) == -1) die("Write (line 131)");
    }

    //  send the final message back (as a string) 
    if(write(fdc[1], gmn_get_message(&gmn), sizeof(char[MSG_BUF_SIZE])) == -1) die("Write (line 136)");
...
int main(int argc, char *argv[])
{
    ...
    if (read(fdc[0], &max, sizeof(int)) == -1) die("Read (line 210)");
    ...
    */
    ...
    do { 
        ...
        if(write(fdp[1], &guess, sizeof(int)) == -1) die("Write (line 225)");
        //     wait for the result from the child
        if(read(fdc[0], &result, sizeof(int)) == -1) die("Read (line 227)");
    ...
    char message[MSG_BUF_SIZE];
    if(read(fdc[0], &message, sizeof(message)) == -1) die("Read (line 241)");
    //printf("%s", message);
